...
class Notification {
  ...
  slack() {
    ...
    let slack = new _slackNode2.default();
  ...
  email() {
    ...
    const emailTemplate = _fs2.default.readFileSync('./email_template.html', 'utf8');

    let mailBody = '';
    for (let i = 0; i < this.reviewDatas.length; i++) {
      // Placeholderを置換する
      mailBody += emailTemplate.replace('{{ appData.name }}', this.appData.name).replace(/{{ appData.url }}/g, this.appData.url).replace('{{ review.title }}', this.reviewDatas[i].title).replace('{{ review.message }}', this.reviewDatas[i].message).replace('{{ review.rating }}', new Array(Number(this.reviewDatas[i].rating) + 1).join('☆')).replace('{{ review.updated }}', this.reviewDatas[i].updated).replace('{{ review.kind }}', this.appData.kind).replace('{{ review.version }}', this.reviewDatas[i].version);
    }
