...
class Review {
  ...
  ios(iosIds) {
    if (iosIds !== null) {
      for (let i = 0; i < iosIds.length; i++) {
        ...
        let iosApp = new _AppData2.default("iOS", iosId);
  ...
  android(androidIds) {
    if (androidIds !== null) {
      for (let i = 0; i < androidIds.length; i++) {
        ...
        let androidApp = new _AppData2.default("Android", androidId);
  ...
  noticeAppReview(appData, url, appfunc) {
    ...
    // アプリのレビューデータを取得
    let param = {};
    _cheerioHttpcli2.default.fetch(url, param, (err, $, res) => {
      if (err) {
        console.log((0, _util.formatDate)(new Date(), "YYYY/MM/DD hh:mm:ss") + " Error:", err);
        return;
      }

      appfunc($, appData).then(reviewData => {

        const notification = new _Notification2.default(appData, reviewData, config);
        // 表示件数制御
  ...
  analyzeIosData($, appData) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let reviewDatas = [];
      (0, _xml2js.parseString)(reviewDataXml, (err, result) => {
  ...
  getIosReview(appData, entry) {
    ...
    return new Promise((resolve, reject) => {
      ...
      param.updated = (0, _util.formatDate)(new Date(entry.updated[0]), "YYYY/MM/DD hh:mm:ss");
      ...
      let reviewData = new _ReviewData2.default(param);
  ...
  getAndroidReview($, appData, element) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const reviewData = new _ReviewData2.default(param);
    ...
    return new Promise((resolve, reject) => {
      this.selectRecord(reviewData, appData.kind).then(result => {
        ...
        if (result.cnt === 0) {
          this.db.serialize(() => {
            // 挿入用プリペアドステートメントを準備
            const ins_androidReview = this.db.prepare("INSERT INTO review(id, kind, app_name, title, message, rating, updated, version, create_date) " + "VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)");

            ins_androidReview.run(reviewData.reviewId, appData.kind, appData.name, reviewData.title, reviewData.message, reviewData.rating, reviewData.updated, reviewData.version, new Date());
  ...
  selectRecord(condition, kind) {
    return new Promise((resolve, reject) => {
      this.db.serialize(() => {
        this.db.get('SELECT count(*) as cnt FROM review WHERE id = $id AND kind = $kind', { $id: condition.reviewId, $kind: kind }, (err, res) => {
          if (err) {
            return reject(err);
          }
          resolve(res);
